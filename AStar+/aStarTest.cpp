...
vector<tile*> aStarTest::addOpenList(tile* currentTile)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)				//  현재 노트 ( 부모노트) 에서 자신을 포함한 주변 9노트 탐색
		{
			tile* node = _vTotalList[(startY * TILENUMX) + startX + j + (i * TILENUMX)];

			if (!node->getIsOpen()) continue;					//이미 오픈노트백터에 있는 노트 거르고
			if (node->getAttribute() == "start") continue;		//시작 지점 노트 거르고
			if (node->getAttribute() == "wall") continue;		//종점 노트 거른다.
			
			if (i == 0 && _vTotalList[(startY * TILENUMX) + startX + j + ((i + 1) * TILENUMX)]->getAttribute() == "wall") continue;
			if (i == 2 && _vTotalList[(startY * TILENUMX) + startX + j + ((i - 1) * TILENUMX)]->getAttribute() == "wall") continue;
			

			node->setParentNode(_currentTile);		// 걸러지고 남은 노트에는 현재 노트에 대한 주소를 넣어준다.

			bool addObj = true;

			for (_viOpenList = _vOpenList.begin(); _viOpenList != _vOpenList.end(); ++_viOpenList)
			{
				if (*_viOpenList == node)		// 이미 그 노트가 오픈노트백터에 넣어져있으면 제외 시켜주는 for 문
				{
					addObj = false;
					break;
				}
			...
			if (node->getAttribute() != "end") node->setColor(RGB(0, 255, 0));		// 걸러지고 남은 노트는 초록색으로 칠해준다.

			if (!addObj) continue;			// 오픈 노트 백터 중복 방지를 위해 컨티뉴

			_vOpenList.push_back(node);		// 오픈 노트 백터에 새로운 오픈 노트를 추가시켜준다.
		}
	}
	return _vOpenList;		//필터링된 오픈노트 리스트를 리턴해준다.
}

void aStarTest::pathFinder(tile* currentTile)
{
	float tempTotalCost = 5000;			//맵의 크기를 감안하여 Cost를 책정한다. 이해가 잘 안되면 100000 이상으로
	tile* tempTile = NULL;

	for (int i = 0; i < addOpenList(currentTile).size(); i++)
	{
		_vOpenList[i]->setCostToGoal((abs(_endTile->getIdX() - _vOpenList[i]->getIdX())
			+ abs(_endTile->getIdY() - _vOpenList[i]->getIdY())) * 10);		//각 오픈노트에서부터 종점까지 거리의 H 비용

		POINT center1 = _vOpenList[i]->getParentNode()->getCenter();
		POINT center2 = _vOpenList[i]->getCenter();

		_vOpenList[i]->setCostFromStart((getDistance(center1.x, center1.y, center2.x, center2.y) > TILEWIDTH) ? 14 : 10);  // g 비용

		_vOpenList[i]->setTotalCost(_vOpenList[i]->getCostToGoal() +
			_vOpenList[i]->getCostFromStart());   // 각 오픈노트의 f = g + h 비용
